...
After each relevant, incoming tweet is intercepted and parsed, it is stored
in MongoDB for further Analysis (NLP).The keyword can be altered at the end of the script.
"""
...
## List of languages (tweets that you want to extract)
LANGUAGES = ['en']

## define a function that can extract geo data of tweets
def get_geo_data(tweet):
    # List of geo data of tweets
    geo_data = []
    if tweet['place'] != None:
        try:
...
## define a function that can extract location of the user / author of each tweet
...
## define a function that can extracts hashtags from each tweet
...
## define a Function that extracts relevant information from the tweet
## (using the 3 user-defined functions from above)
...
## Created a Class for Authentication
...
## Created a Class that inherits from tweepy.StreamListener Class
